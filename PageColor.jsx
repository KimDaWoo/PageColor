...
function PageColor() {
    ...
    return (
        <ColorContext.Provider value={{ isDark, setIsDark }} > <Page />
...
function Header() {
    const { isDark } = useContext(ColorContext); return (
        <header className='header' style={{
            backgroundColor: isDark ? 'black' : 'lightgray',
            color: isDark ? 'white' : 'black',
        }}>
            <h2>컨텍스트 사용강의</h2> </header>
    )
}
...
function Content() {
    ...
    return (
        <content className='content'
            style={{
                backgroundColor: isDark ? 'black' : 'lightgray', color: isDark ? 'white' : 'black',
            }}>
